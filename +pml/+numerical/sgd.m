...
function [x, report] = sgd(func, x0, data, options)
  ...
  % set to 1 to enable
  [convergence_check, options] = option(options, 'ConvergenceCheck', 1);
  ...
  % NeighborhoodSystem, control the dimension of filter
  [tempNS, options] = option(options, 'NeighborhoodSystem', 'N2');
  % Number of convolution filters
  [tempNF, options] = option(options, 'FilterNumber', 4);
  % Number of GSM scales
  [tempNG, options] = option(options, 'GSMScale', 7);
  % The path of current project
  [ProjectPath, options] = option(options, 'ProjectPath', 'NONE');
  % Set the save model folder
  ModelName = ['CNN_',tempNS,'_','FN',num2str(tempNF,'%02d%'),'_GSM',num2str(length(options.GSMScale),'%02d%')];
  ModelFolder = fullfile(ProjectPath,ModelName);
  % Get the model initial value
  cnn = options.Model;
%   [cnn, options] = option(options, 'Model', 'NONE');
  if verbosive
  ...
  for batch = startbatch:maxbatches
    ...
    for i = 1:minibatchsize:ndata
      ...
      fprintf('\rBatch: %03d, Minibatch: %04d', batch, iter);
      iter = iter + 1;
    ...
    % save current training model
    if mod(batch,2) == 0
      if ~exist(ModelFolder,'file')
        mkdir(ModelFolder);     
      end
      
      theta = x;      
      nweights = length(cnn.weights);
      GSMweights = theta(1:nweights);
      J_tilde = theta(nweights+1:end);
      % set estimated weights (converted back from log-space)
      cnn.weights = exp(GSMweights);
      % set filter
      cnn.J_tilde(:) = J_tilde;
      
      save(fullfile(ModelFolder,[ModelName,'_',num2str(batch,'%02d'),'.mat']), 'cnn','-v7.3')
    end
