...
classdef cnn < pml.distributions.density
  %% Abstract constants
  ...
  %% Main methods
  methods
    function this = cnn(varargin)
    %CNN - Constructs Convolutional Neural Networks
...
%       error(nargchk(0, 1, nargin));
    ...
    function this = set.filtertype(this,filtertype)
    ...
    %SET_FILTER - Set all filter parameters at once
      ...
      if this.zeromean_filter
        this.J = bsxfun(@times, this.J, this.filtertype);
        this.J = bsxfun(@minus, this.J, sum(this.J)./sum(this.filtertype));
        this.J = bsxfun(@times, this.J, this.filtertype);
      end
      % update filter matrices
      this = this.update;
